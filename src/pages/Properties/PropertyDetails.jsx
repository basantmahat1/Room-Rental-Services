  ...
  {allImages.length > 0 ? (
    <>
      ...
      <div className="relative mb-4 shadow-xl rounded-2xl overflow-hidden bg-gray-100 group">
        <div className="aspect-[16/9] w-full max-h-[600px] overflow-hidden">
          <img
            key={selectedImage}
    ...
    <div className="aspect-[16/9] bg-gray-50 border-2 border-dashed border-gray-200 rounded-2xl flex flex-col items-center justify-center">
       ...
       <p className="text-gray-400 font-medium">No images uploaded yet</p>
                ...
                {allImages.length > 4 && (
                  ...
                  >
                    🖼️ View All {allImages.length} Photos
                  </button>
                )}
              </>
            ) : (
              <div className="card w-full h-[500px] flex items-center justify-center">
                <div className="text-center">
              ...
              <div className="card p-6">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
                  <div className="flex-1">
                    <h1 className="text-3xl font-bold mb-2" style={{color: 'var(--color-primary)'}}>{property.title}</h1>
                ...
                <div className="flex flex-wrap gap-2 mb-4">
                  ...
                  {property.instant_booking && (
                    <span className="badge-info">⚡ Instant Booking</span>
                  )}
                  {property.furnishing && (
                    <span className="badge-warning capitalize">{property.furnishing}</span>
                  )}
                  <span className="badge-info capitalize">{property.property_type}</span>
                </div>

                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 p-4 rounded-lg" style={{backgroundColor: 'var(--color-bg-light)'}}>
                  <div className="text-center">
                    <div className="text-2xl font-bold mb-1" style={{color: 'var(--color-primary)'}}>{property.bedrooms}</div>
                    <div className="text-sm text-gray-600">Bedrooms</div>
                  </div>
                  <div className="text-center">
                    <div className="text-2xl font-bold mb-1" style={{color: 'var(--color-primary)'}}>{property.bathrooms}</div>
                    <div className="text-sm text-gray-600">Bathrooms</div>
                  </div>
                  <div className="text-center">
                    <div className="text-2xl font-bold mb-1" style={{color: 'var(--color-primary)'}}>{property.area_sqft}</div>
                    <div className="text-sm text-gray-600">Sq. Ft.</div>
                  </div>
                  <div className="text-center">
                    <div className="text-2xl font-bold mb-1" style={{color: 'var(--color-primary)'}}>{property.view_count || 0}</div>
              ...
              {property.latitude && property.longitude && (
                <div className="card p-6">
                  <div className="flex justify-between items-center mb-4">
                    <h2 className="text-xl font-bold" style={{color: 'var(--color-primary)'}}>📍 Location</h2>
            ...
            <div className="space-y-6">
              <div className="card p-6 sticky top-4">
                ...
                <div className="flex items-center gap-4 mb-4">
                  ...
                  <div>
                    <div className="font-semibold text-lg">{property.owner_name || 'Owner'}</div>
                    {property.owner_verified && (
                      <div className="text-sm flex items-center gap-1" style={{color: 'var(--color-success)'}}>
                        <span>✓</span>
                ...
                {property.owner_phone && (
                  <div className="mb-6">
                    ...
                    <div className="font-medium">{property.owner_phone}</div>
                  </div>
                )}
                ...
                <div className="space-y-3">
                  {isAuthenticated && user?.role === 'tenant' ? (
                    <>
                      <button onClick={() => setShowBookingModal(true)} className="btn-primary w-full">
                  ...
                  {property.owner_phone && (
                    <button onClick={handleWhatsAppContact} className="w-full bg-green-500 text-white py-3 rounded-lg font-semibold hover:bg-green-600 transition-colors">
                      💬 WhatsApp Owner
                    </button>
                  )}
              ...
              {(property.avg_rating && property.avg_rating > 0) && (
                <div className="card p-6">
                  <h3 className="font-bold text-lg mb-4" style={{color: 'var(--color-primary)'}}>⭐ Ratings & Reviews</h3>
                  <div className="text-center">
                    ...
                    <div className="flex justify-center mb-2">
                      {[...Array(5)].map((_, i) => (
                        <span key={i} className={`text-2xl ${i < Math.floor(property.avg_rating) ? 'text-yellow-400' : 'text-gray-300'}`}>
                          ★
                        </span>
                      ))}
                    </div>
                    <div className="text-sm text-gray-600">
                      Based on {property.total_reviews || 0} review(s)
                    </div>
                  </div>
                </div>
              )}
